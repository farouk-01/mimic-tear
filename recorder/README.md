...
```powershell
.\.venv\Scripts\python.exe recorder\record.py `
  --theme exploration `
  --tag basic-movement `
  --split train `
  --label movement
...
.\.venv\Scripts\python.exe recorder\record.py --theme combat --tag boss-practice --split train --monitor 1

# Capture a desktop region expressed as left,top,right,bottom.
.\.venv\Scripts\python.exe recorder\record.py --theme exploration --tag interior-01 --split train --region 100,80,1820,1040

# Limit a headless collection run to ten minutes.
.\.venv\Scripts\python.exe recorder\record.py --theme combat --tag group-01 --split train --no-preview --max-duration 600

# Cancel and discard an in-progress recording with CTRL+F9.
.\.venv\Scripts\python.exe recorder\record.py --theme movement --tag basic-movement --split train --cancel-hotkey CTRL+F9

# Also write a human-readable CSV mirror.
.\.venv\Scripts\python.exe recorder\record.py --theme items --tag healing-01 --split validation --csv
```
...
To use the loop for another encounter, invoke `recorder\record.py` directly
...
- `game_state.parquet`: optional frame-aligned, read-only process-memory state
  created only when `--game-state-profile` is supplied.
...
## Optional read-only Elden Ring state
...
The bundled [Elden Ring profile](profiles/elden_ring.json) discovers the
...
```powershell
.\.venv\Scripts\python.exe recorder\discover_elden_ring.py
```

Test a filled profile with a single read before recording:

```powershell
.\.venv\Scripts\python.exe recorder\probe_game_state.py `
  recorder\profiles\elden_ring.json
```
...
.\.venv\Scripts\python.exe recorder\record.py `
  ...
  --split train `
  --label lock-on `
  --game-state-profile recorder\profiles\elden_ring.json
```
...
String fields require a fixed maximum `length`.

The fixed state schema includes:

- Player health, FP, stamina, position, and camera orientation.
- Lock-on status plus the current target's ID, name, health, and position.
- Location ID and location name.

Fields without enabled or currently readable pointers are null. Optional
lock-on target fields can therefore remain null while unlocked. A failed
required field makes that snapshot invalid. The first snapshot must be valid
or recording aborts; later invalid samples are retained with
`state_valid=false` and reported by validation. Every state row has the same
frame index and frame timestamp as its controller row.
...
```powershell
.\.venv\Scripts\python.exe recorder\validate.py recordings\train\exploration\basic-movement
```
...
```powershell
.\.venv\Scripts\python.exe recorder\replay.py recordings\train\exploration\basic-movement
```
