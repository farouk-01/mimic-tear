...
import torch
from data.models.game_state import GameStateField, GameStateSchema
...
class ProcessedGameStateSchema(GameStateSchema):
    ...
    @classmethod
    def from_json(cls, version: int = SCHEMA_VERSION) -> ProcessedGameStateSchema:
        import json

        data = json.loads(
            _get_processed_schema_path(version).read_text(encoding="utf-8")
        )
