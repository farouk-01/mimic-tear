...
from ai_player.paths import DEFAULT_GAME_STATE_PROFILE, PROJECT_ROOT
...
from ai_player.recording.synchronization import FrameClockSynchronizer  # noqa: E402
from ai_player.recording.game_state_capture import (  # noqa: E402
    GameStateSampler,
    GameStateWriter,
)
from ai_player.platform.windows.global_hotkey import GlobalHotkey, parse_hotkey  # noqa: E402
...
def finalize_session(
    ...
    from ai_player.recording.validation import validate_session
...
def open_replay(session_directory: Path) -> None:
    from ai_player.recording.replay import load_replay_session, run_replay
...
def main() -> None:
    try:
        config = parse_args()
        if config.boss_loop:
            from ai_player.recording.boss_loop import run_boss_recording_loop
