...
from mimic_tear.paths import DEFAULT_GAME_STATE_PROFILE, PROJECT_ROOT
...
from mimic_tear.recording.synchronization import FrameClockSynchronizer  # noqa: E402
from mimic_tear.recording.game_state_capture import (  # noqa: E402
    GameStateSampler,
    GameStateWriter,
)
from mimic_tear.platform.windows.global_hotkey import GlobalHotkey, parse_hotkey  # noqa: E402


PREVIEW_TITLE = "Mimic Tear Recorder"
...
class RecorderConfig:
    ...
    game_state_profile: Path | None = DEFAULT_GAME_STATE_PROFILE
    ...
    boss_reset_profile: Path = DEFAULT_GAME_STATE_PROFILE
...
def finalize_session(
    ...
    from mimic_tear.recording.validation import validate_session
...
def open_replay(session_directory: Path) -> None:
    from mimic_tear.recording.replay import load_replay_session, run_replay
...
def parse_args(argv: list[str] | None = None) -> RecorderConfig:
    ...
    parser.add_argument(
        ...
        help="read-only game-state profile (enabled by default)",
    )
    parser.add_argument(
        "--no-game-state",
        action="store_true",
        help="disable frame-aligned game-state capture",
    )
...
def main() -> None:
    try:
        config = parse_args()
        if config.boss_loop:
            from mimic_tear.recording.boss_loop import run_boss_recording_loop
