...
type FeatureSize = Annotated[int, Field(gt=0)]


class VectorFusionConfig(BaseModel):
    model_config = ConfigDict(frozen=True, extra="forbid", strict=True)

    input_features: tuple[FeatureSize, ...] = Field(min_length=1)
    ...
    def __init__(
        ...
        *,
        ...
        if not input_features:
            raise ValueError("Expected at least one input feature size")

        if any(size <= 0 for size in input_features):
            raise ValueError("Input feature sizes must be greater than zero")
        ...
        if len(input_features) == 1 and input_features[0] == output_features:
            ...
            self.fusion = nn.Sequential(
                nn.Linear(self.total_input_features, output_features),
